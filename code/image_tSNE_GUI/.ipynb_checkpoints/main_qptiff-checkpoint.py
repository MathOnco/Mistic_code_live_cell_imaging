...
## this is main.py that works for qptiff data
## we needed to modify weights, and how to read in gen_image_tsne()
## tag is 15th feb 2022
## added the dropdown for codex qptiff so all this is in place
## a. the shuffle option is not in order
## so just use no shuffle
## b. imread for rest of the options need to be added
...
def generate_image_tSNE(chk_box_marker,rb_val,rb_rs_val,rb_shf_val, rb_imtech_val, mc, wc, LABELS_MARKERS):
    ...
    if (rb_shf_val == 0): # no shuffle option
        ...
        for k in range(len(inds)):
            ...
            ##remember we need to open this for the rest of the imaging techs
            ## this code is purely for qptiff
            if (rb_imtech_val ==2): #for codex
                im = tifffile.imread(marker_image_list[inds[k]])
                im = im.reshape(64,5040,9408) ##hardcoded cycif remove # 11th feb 2022
                
            #elif(rb_imtech_val ==4): # for codex qptiff 15th feb 2022
            k_key = [50,53,56, 58, 61,62]
                #image_all_1 = image_all_2 = []
                
            
            #print (im.shape)    
            
            #for m in range(len(marker_choice)): #15th feb 2022
            for m in range(len(k_key)):
                image = tifffile.imread(marker_image_list[inds[k]], key = k_key[m])
            ...
            if (rb_imtech_val ==0): #vectra
                ...
                tile_1 = Image.fromarray(np.uint8(cm.jet(tl)*255)) #20th jan 2022 #500?
                 ...
                 tile_1 = Image.fromarray(np.uint8(cm.jet(tl)*255)) #20th jan 2022
            elif (rb_imtech_val ==3): # CODEX QPTIFF
                 tile_1 = Image.fromarray(np.uint8(cm.jet(tl)*255)) #20th jan 2022
...
def create_figure(stack_montage_flag):
    ...
    if (rb_imtech == 'Vectra'):
        ...
        rb_imtech_val = 2
    elif (rb_imtech=='CODEX QPTIFF'): #15th Feb 2022
        rb_imtech_val = 3
    ...
    if (stack_montage_flag==True):
        ...
        elif (rb_imtech_val ==3):#codex qptiff #15th Feb 2022
            wc = [30] * len(LABELS_MARKERS)      
            wc[1] = 100 #was 50 pre codex 11th feb 2022
            wc[2] = 50  
            wc[3] = 300
            wc[5] = 140
...
RB_4 = ['Vectra', 't-CyCIF', 'CODEX', 'CODEX QPTIFF'] #14th feb 2022, 15th feb 2022
